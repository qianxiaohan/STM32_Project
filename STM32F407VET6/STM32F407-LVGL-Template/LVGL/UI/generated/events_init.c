...
static void screen_sw_1_event_handler (lv_event_t *e)
{
    ...
    case LV_EVENT_VALUE_CHANGED:
    {
        ...
        {
            GPIO_ResetBits(GPIOA, GPIO_Pin_6);    //µãÁÁLED
        }
        else
        {
            GPIO_SetBits(GPIOA, GPIO_Pin_6);    //Ï¨ÃðLED
        }
        break;
    }
    default:
...
static void screen_sw_2_event_handler (lv_event_t *e)
{
    ...
    case LV_EVENT_VALUE_CHANGED:
    {
        if(lv_obj_has_state(obj, LV_STATE_CHECKED))
        {
            GPIO_ResetBits(GPIOA, GPIO_Pin_7);    //µãÁÁLED
        }
        else
        {
            GPIO_SetBits(GPIOA, GPIO_Pin_7);    //Ï¨ÃðLED
        }
        break;
    }
    default:
