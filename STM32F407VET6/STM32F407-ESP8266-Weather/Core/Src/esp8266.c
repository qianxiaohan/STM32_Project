...
const char *config_sntp = "AT+CIPSNTPCFG=1,8,\"cn.ntp.org.cn\",\"ntp.sjtu.edu.cn\"\r\n"; // 配置SNTP服务器
const char *get_sntp_time = "AT+CIPSNTPTIME?\r\n";										 // 获取STNP时间

/* API可配置的参数 */
const char *key = "#######"; // 心知天气API私钥，免费申请即可
const char *location = "chengdu";	   // 地区
const char *days = "1";				   // 查询1~3天的天气，这里只实现了解析未来1天的天气

/* 用于解析日期的缓冲区 */
uint8_t date_buffer[64];

static void RTC_CalendarConfig(void);	//配置RTC
...
static void RTC_CalendarConfig(void)
{
	...
	if (times != 0)
	{
		...
		if(parse_date((char*)date_buffer, &dt) == 1)
		{
			...
			sdatestructure.Month = getMonthNumberCaseInsensitive(dt.month);
			sdatestructure.Date = intToBCD(dt.day);
			sdatestructure.WeekDay = getWeekNumberCaseInsensitive(dt.week);
